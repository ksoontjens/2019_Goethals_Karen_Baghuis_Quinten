...
public class HelloTVXlet implements Xlet, UserEventListener
{
    ...
    public void userEventReceived(UserEvent e) {
        ...
        if(e.getCode() == HRcEvent.VK_UP && mc.sx != 125 && e.getType() == HRcEvent.KEY_PRESSED)
        {
            mc.sx = mc.sx - 100; mc.repaint();
        }
        if(e.getCode() == HRcEvent.VK_DOWN && mc.sx != 325 && e.getType() == HRcEvent.KEY_PRESSED)
        {
            mc.sx = mc.sx + 100; mc.repaint();
        } 
        if(e.getCode() == HRcEvent.VK_NUMPAD0 && e.getType() == HRcEvent.KEY_PRESSED)
        {
            mc.dead = false;
